...
doInline (Image (label, ["mermaid"], opts) caption _) = do
  doMermaid label opts "ERROR: No source" caption
doInline i = pure i

doInclude b = walkM doBlock b

doMermaid label opts0 md caption = do
  (md,opts) <- extractSource md opts0
