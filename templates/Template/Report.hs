...
templateLatex = do
  let (styFile,styText) = templateSty
  T.writeFile ("_build/" ++ styFile ) styText
  mainTemplate

mainTemplate :: IO String
mainTemplate = return [r|
...
$if(affiliation)$
$for(affiliation)$
  \lighteraddress{$$^$affiliation.idx$$$}{{\em $affiliation.aff$}}\\
$endfor$
$endif$
...
\newpage
\appendix
\input{$for(appendix)$$appendix$$sep$,$endfor$}%
$endif$
%\newpage
%\appendix
...
templateSty = ( "paperlighter.sty",  [r|
